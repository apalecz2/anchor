...
|---|---|---|---|
| `features/extraction/provenance.ts` | 🔴 | `matchCellsToOcr`, `matchFromCursor`, `fuzzyMatchPass`, `gridMatchPass`, `getCellSourceBox`, `sanitizeWordsForProvenance`, `levenshtein/similarity/normalize` | *Has tests — extend.* 12-word lookahead boundary (match at edge vs just beyond); multi-word cell spanning exactly the window; cursor never rewinds; fuzzy bounds cannot steal a word an adjacent cell already claimed `(CR:F2)`; fuzzy lower bound = prevMatch+1 / upper = nextMatch; 0.8 threshold edge (0.79 reject / 0.80 accept); empty CSV, empty OCR, all-unmatched row; UUID stability after reorder/delete `(CR:H2)`; pipe-glyph sanitize keeps pipe-only reads. **Grid cross-check `(CR:F2)`:** a reading-order-desynced cell is recovered from its row band (matched siblings) ∩ column band (other matched rows); no recovery when a whole column has no anchor; an in-band but sub-threshold candidate stays `unmatched`; a claimed word is never re-stolen. **Grounding dispatch:** the default (`word`) path is byte-identical to before; a model-**declared** grid is used whenever one is supplied, independent of the grounding tier (the words-plus-declared-grid pairing is the one that ships); a declared grid places cells no inferred grid could — the fixture uses *abutting* columns so `detectColumnSeparators` genuinely finds nothing, which is what gives the test teeth; a declared grid whose column count disagrees with the TSV, or whose bands contain none of the words, declines back to *inference*; `block`/`none` infer no grid at all |
| ~~`features/llama/contextBudget.ts`~~ | — | — | **Moved to Rust** (`pipeline/budget.rs`, §6.6a) when the executor took over orchestration. Its cases moved with it, and it now reads the context size from the catalog instead of mirroring a constant. |
| `features/settings/customModel.ts` | 🟡 | `fileNameOf`, `formatModelSize` | Windows and POSIX paths; trailing separators; a bare filename; **falls back to the input rather than returning empty** (a settings row showing nothing where a path should be reads as a bug). Sizes lead with GB; an unrecorded size says "unknown size" rather than claiming the file is empty. Validation is deliberately *not* duplicated here — Rust owns it (§6.6a). |
| `features/extraction/confidence.ts` | 🔴 | `parseTSVWithOffsets`, `mapLogprobsToCells`, `cellTrust`, `computeProvenanceCells` | *Has tests — extend.* TSV: opening/closing fences, CRLF, blank rows between data, trailing tab → empty cell, single column, ragged rows; **char-offset alignment** — a logprob at offset X maps to the cell whose `[start,end)` contains it; `cellTrust` every branch: `disagree`→low, `image_only` ≥0.85→medium else low, blended ≥0.85 ∧ llmMin≥0.5→high, blended<0.5 gate keeps high out, ≥0.65→medium, else low; `computeProvenanceCells`: null logprobs excluded from mean (not treated as prob 1.0) `(CR:M17)`, OCR confidence resolved by UUID, `ocr=null` when unmatched, fuzzy knockdown high→medium / else→low. **Generalized agreement axis:** an `agree` cell with only one numeric signal scores on that one, *uncapped* — blending the missing term in as zero caps it at 0.4 and renders a correctly-matched table entirely red, silently; `self_reported` runs the same ladder with the one source standing in for both terms and caps at `medium`; it is more generous than `image_only` at the same score, because absence of evidence is not evidence against; a model-grounded, model-verified cell still reaches `high`; an ungrounded blank cell is `medium`, not the `high` a real spatial check earns |
...
| ~~`features/llama/llamaClient.ts`~~ | — | **Moved to Rust.** Model I/O is `pipeline/client.rs` + `pipeline/executor.rs` (§6.6a); what remains in TypeScript is `stopLlamaServer`. The properties did not vanish with the file: SSE parsing, multi-token logprobs and the missing-logprob→`None` rule are now tested against recorded frames in `client.rs`, and the CR:M8 health-shape check (a bare 200 is not proof the responder is llama-server) is `is_healthy_body` in `executor.rs`. |
| `features/settings/CustomModelSection.tsx` | 🟡 | The unverified warning shows **whether or not** a model is chosen (before, it is what the user is agreeing to; after, it is why a bad result may not be a bug); the registered model renders by filename and size; picking a file registers it through the backend; **the backend's rejection is surfaced verbatim** — it names which check failed, and a generic message would turn a fixable mistake into a mystery; a dismissed dialog registers nothing; the user can return to the built-in model; an unreadable registration reads as "none chosen" rather than breaking the page |
...
#### 6.6a `pipeline/` — the manifest-driven extraction pipeline (Tier 3)

The subtree that took over orchestration from the frontend. Everything here is pure or
filesystem-only, so it is all Tier 3; the parts that need a live `llama-server` are
covered by the manual checks in §9 instead.

| File | Pri | Cases |
|---|---|---|
| `pipeline/catalog.rs` | 🔴 | `validate_catalog` against **deliberately broken** catalogs — it takes its inputs as parameters precisely so this is possible, and most of the suite asserts rejection: a model used for a role it does not hold; an unknown model id; a preset not starting with render; zero or two item-grounding steps; no structure step; a verify before any structure; a grid step before grounding, after structuring, or duplicated; a grid step on a model that reports only block boxes; a model claiming `Ground` with no locations; a RAM floor below its own models'; duplicate ids. Plus: the bundled catalog validates; prompts are non-empty and Surya's are pinned **verbatim** (they are training-time contracts); the TSV prompt ends with the load-bearing `\n\nOCR text:\n`; the text-only prompt mentions no image; Qwen's spec still mirrors the values that shipped before the refactor; steps serialize with their `kind` tag (a wire format shared with the frontend). |
| `pipeline/prompt.rs` | 🔴 | **Golden-file equivalence with `ocrTransforms.ts`** — the same fixture through both languages, compared byte for byte. Explicit cases for the three fidelity rules: UTF-16 (not UTF-8) string lengths, JavaScript's `Math.round` tie-breaking, stable sorts. Verified to have teeth by mutation: switching `utf16_len` to bytes must fail both the golden test and the non-ASCII assertion. |
| `pipeline/client.rs` | 🔴 | SSE parsing against **recorded frames** — the response shape most likely to shift under a llama.cpp upgrade. Chunk-boundary reassembly, CRLF, keepalives, `[DONE]`, malformed frames, a missing logprob → `None` (never `0.0`, which would inflate trust for exactly the tokens with no signal), and `charOffset` in UTF-16 units. Request bodies: every sampling field from the catalog, optional keys omitted rather than sent as null. |
| `pipeline/budget.rs` | 🟡 | Ported `contextBudget` cases; image tokens charged only for a vision model; `resolve_max_tokens` with and without the boost. |
| `pipeline/surya.rs` | 🔴 | Band JSON parsed tolerantly (fences, wrapper objects, prose around the array, `bbox` as array or string, alternate key spellings); unusable entries **counted** rather than silently dropped; unparseable output yields empty bands, never a panic. Grid construction: the P0 spike's real 13 × 6 bands; sorting into reading order; fewer than two columns is not a grid; degenerate bands dropped while the rest survive; inverted boxes normalized rather than producing negative extents; clamping to the page. Block HTML: nested `<div>` not splitting its parent, truncated responses keeping completed blocks, stray close tags, `<divider>` not matching `<div>`, entity decoding, and **offsets in UTF-16 code units** (a non-ASCII earlier block is how a silent misalignment gets in). |
| `pipeline/custom.rs` | 🔴 | **Every URL shape refused** (`https`, `http`, `file`, `ftp`, `smb`, mixed case) — the boundary the module exists to hold; a Windows drive letter is *not* a URL scheme; relative paths, missing files, directories, wrong extensions refused; **contents checked against the `GGUF` magic bytes**, so a renamed executable, a text file, a truncated or empty file are all refused with a message naming the reason. Registration round-trips through camelCase JSON, and an older registration missing optional fields still loads. |
| `pipeline/executor.rs` | 🔴 | Run lifecycle: ids monotonic; beginning a run cancels the one it supersedes; a **stale `finish` must not clear a newer run** (it would become uncancellable). `is_healthy_body` — a bare 200 is not proof the responder is llama-server, since the ephemeral port is released a moment before the spawn binds it (carried over from the deleted `llamaClient` test, CR:M8). Step labels name the model actually running and degrade to a generic string for an unknown one rather than unwrapping. Every step kind the shipped **and pending** presets use is one `run_page` handles. An unusable band response yields no grid rather than a bad one. |
...
This suite **deliberately violates the §1 Determinism principle** ("no test depends on a real model, network, wall-clock timing, or GPU"): its entire purpose is to measure the real pipeline's accuracy, so it needs the real Tesseract/oar-ocr/Qwen models actually installed (a completed first-run setup, not the fixture asset server the journeys above use) and real LLM inference. (Surya's presets were part of this comparison until 2026-09-25, when they were pulled from `catalog::PRESETS` — see `docs/design.md` §5 — after this very suite found Surya's grid step added nothing paired with an LLM and lost to it used alone; `e2e/eval/presets.ts` no longer lists them.) For that reason it is:
