...
| Tier | Preset | What it runs |
|---|---|---|
| 8 GB | `tesseract-qwen3.5-4b` (**Fast**, default) | Tesseract grounds the page, Qwen3.5 4B builds the table. Today's behaviour. |
| 16 GB | `tesseract-surya-qwen3.5-4b` (**Accurate**) | Tesseract's words, Surya's row/column grid, Qwen3.5 4B's table. |

The Accurate preset is **built but not yet offered**: it names a model whose downloads are not pinned, and a model the catalog offers while the installer cannot verify it is exactly what §7.1's pinning invariant exists to prevent. It is defined and validated by tests in the meantime so it cannot rot while it waits; it joins the list in the same change that adds its SHA-256 pins.
...
**The pipeline is data.** A *preset* is an ordered list of typed steps — render, ground, map the grid, structure, verify — naming models from a catalog of what each model is allowed to do. `pipeline/executor.rs` walks those steps for one page and returns a `PageArtifact`; nothing below is a hardcoded call site. The numbered stages that follow describe the **default preset** (`tesseract-qwen3.5-4b`), which reproduces the behaviour the app has always had; where another preset differs, it is called out.
...
3a. Grid mapping *(optional; the Accurate preset only — see §5 for its status)*
...
5. Confidence scoring
   ...
   - Grounding confidence: mean confidence of the matched source items — Tesseract's per-word score today; `null` for unmatched cells.
