...
- [ ] Mirror oar-ocr's PP-OCRv6 model files (det/rec/dict, ~30MB) to Anchor's own R2 bucket and pin their SHA-256 through `setup.rs`'s manifest, the way every other model asset already works -- this is what lifts `oar-ocr-qwen3.5-4b` out of `#[cfg(debug_assertions)]` and into a release build; today it relies on the `oar-ocr` crate's own live `auto-download` from ModelScope, unpinned by this app (see `docs/design.md` §5, `NOTICES.md` §1.7)
- [ ] `ocr.rs::run_oar_ocr` rebuilds the entire oar-ocr pipeline (`OAROCRBuilder::new(...).build()`) from scratch on every page, including every page of a multi-page PDF -- no session/engine reuse across pages the way a resident `llama-server` is reused across steps. Was a correctness-first shortcut when oar-ocr was first wired in; worth caching once the engine is trusted, especially for long documents
...
- [x] Pulled `tesseract-surya-qwen3.5-4b`, `oar-ocr-surya-qwen3.5-4b`, and `oar-ocr-surya-no-llm` out of `catalog::PRESETS` (2026-09-25) -- no longer offered, recommended, or selectable in *any* build, debug included. Two independent reasons converged: (1) legal -- Surya's weights ship under a modified OpenRAIL-M license with a competing-product ban carrying no revenue exemption, a share-alike clause whose text reaches Anchor's own *output*, and a mandatory EULA pass-through, none of which `NOTICES.md`/`EULA.md` accounted for; (2) accuracy -- the real `e2e/eval` harness (12-image seeded sample against the ground-truth corpus) found Surya's grid step byte-identical-to-no-effect when paired with Qwen (`oar-ocr-qwen3.5-4b` vs. the old `oar-ocr-surya-qwen3.5-4b`, both 0.477 mean cellAccuracy -- the grid never reaches the Structure prompt) and materially worse used alone (`oar-ocr-surya-no-llm`, 0.304 mean). `SURYA_OCR_2`, the three presets, `Step::GroundGrid`, and `pipeline/surya.rs` all remain in the codebase as tested interface code; re-offering any of them needs real legal clearance first, a re-measured accuracy case second. Supersedes the `recommend_preset`-ranks-an-unpinned-Surya-preset landmine and the `oar-ocr-surya-no-llm` coverage gaps this list used to track -- both are moot now that the preset isn't reachable at all
