...
- [ ] `hardware::recommend_preset` still ranks `oar-ocr-surya-qwen3.5-4b` first for any machine reporting ≥16GB RAM, and `SetupWizard.tsx`'s Automatic mode now explicitly requests it (`presetId: info.recommended_preset`) -- so until the R2 mirroring above (this item and Surya's own, tracked separately) lands, a fresh Automatic install on a ≥16GB machine will recommend and attempt a preset whose assets can't download, hitting the same 404 `docs/issues.md`'s post-mortem documents. Not a lockout anymore -- the failure screen now offers a working preset instead (see Completed) -- but the first-run failure itself still isn't prevented. Either exclude debug-only/unpinned presets from `recommend_preset`'s candidates, or reorder `PRESETS` so Automatic never lands on one by default
...
- [ ] Click-to-highlight for `oar-ocr-surya-no-llm` (`Step::AssembleFromGrid`) is unverified -- `docs/design.md` §6 step 4a asserts the synthesized TSV round-trips through the normal grid-first provenance matcher with no special case needed, since it's built from the same words the matcher would re-derive, but nobody has actually clicked a cell in a running session to confirm a highlight lands on the right words. Confirm manually or add a `provenance.ts` test before relying on this claim
