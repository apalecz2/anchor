...
import type { AssetManifestEntry, HardwareInfo, SetupConfig, SetupStep } from './types';
...
const STEP_ORDER: SetupStep[] = ['welcome', 'hardware', 'config', 'download', 'verify', 'complete'];
            ...
            <div className="flex-1 overflow-y-auto flex items-start justify-center p-8">
                <div className="w-full max-w-2xl">
                    {errorMsg ? (
                        ...
                        <>
                            {step === 'welcome' && (
                                <WelcomeStep onNext={() => setStep('hardware')} />
                            )}
                            {step === 'hardware' && (
                                <HardwareStep
                                    onNext={info => { setHardware(info); setStep('config'); }}
