...
fn ocr_image_to_page(image_path: &Path, natural_width: i32, natural_height: i32) -> Result<DocumentPageResult, String> {
    let mut args = rusty_tesseract::Args::default();
    args.lang = "eng".to_string();

    let tesseract_image = rusty_tesseract::tesseract::input::Image::from_path(image_path)
...
async fn process_document(
    ...
    if extension == "pdf" {
        ...
        pages.push(ocr_image_to_page(source_path, natural_width, natural_height)?);
